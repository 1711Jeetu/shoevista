...
<div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
    <Link to={`/products/${product.id}`}>
        <img className="p-8 rounded-t-lg" src={product.imageURL[0].URL} alt="product" />
    </Link>
    <div className="px-5 pb-5">
        ...
        <Link to={`/products/${product.id}`}>
            ...
            {console.log(product)}
            {/* <div className="font-semibold tracking-tight text-gray-900 dark:text-white mt-2 mb-2">Items left - <span>{product.items_left}</span></div> */}
